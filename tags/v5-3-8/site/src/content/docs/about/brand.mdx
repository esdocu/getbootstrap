---
title: Directrices de marca
description: Documentación y ejemplos para el logotipo de Bootstrap y las directrices de uso de la marca.
toc: true
---

¿Necesitas los recursos de marca de Bootstrap? ¡Excelente! Solo seguimos unas pocas pautas y, a su vez, te pedimos que también las sigas.

## Logotipo

Al hacer referencia a Bootstrap, utiliza nuestra marca de logotipo. No modifiques nuestros logotipos de ninguna manera. No utilices la identidad de marca de Bootstrap para tus propios proyectos de código abierto o cerrado.

<div class="bd-brand-item px-2 py-5 mb-3 border rounded-3">
  <img class="d-block img-fluid mx-auto" src="/docs/[[config:docs_version]]/assets/brand/bootstrap-logo.svg" alt="Bootstrap" width="256" height="204" />
</div>

Nuestra marca de logotipo también está disponible en blanco y negro. Todas las reglas para nuestro logotipo principal también se aplican a estos.

<div class="bd-brand-logos d-sm-flex text-center bg-light rounded-3 overflow-hidden w-100 mb-3">
  <div class="bd-brand-item w-100 px-2 py-5">
    <img src="/docs/[[config:docs_version]]/assets/brand/bootstrap-logo-black.svg" alt="Bootstrap" width="128" height="102" loading="lazy" />
  </div>
  <div class="bd-brand-item w-100 px-2 py-5 inverse">
    <img src="/docs/[[config:docs_version]]/assets/brand/bootstrap-logo-white.svg" alt="Bootstrap" width="128" height="102" loading="lazy" />
  </div>
</div>

## Nombre

Bootstrap siempre debe ser referido simplemente como **Bootstrap**. Sin la _S_ mayúscula.

<div class="bd-brand-logos d-sm-flex text-center border rounded-3 overflow-hidden w-100 mb-3">
  <div class="bd-brand-item w-100 px-2 py-5">
    <div class="h3">Bootstrap</div>
    <strong class="text-success">Correcto</strong>
  </div>
  <div class="bd-brand-item w-100 px-2 py-5">
    <div class="h3 text-body-secondary">BootStrap</div>
    <strong class="text-danger">Incorrecto</strong>
  </div>
</div>
